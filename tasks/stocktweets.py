...
def get_dataset(train, valid, test, tokenizer, max_len, num_labels):
    data_train  = DatasetTweets(X=train[0], y=train[1], tokenizer=tokenizer, max_length=max_len, num_labels=num_labels)
    data_valid  = DatasetTweets(X=valid[0], y=valid[1], tokenizer=tokenizer, max_length=max_len, num_labels=num_labels)
    data_test  = DatasetTweets(X=test[0], y=test[1], tokenizer=tokenizer, max_length=max_len, num_labels=num_labels)
