...
def get_params(**kwargs):
    ...
    params = {
        ...
        "epochs":20,
        "do_lower_case":True,
        "path":"./data/risk/groundTruth.dat",
    }
...
def load_file(seed, path):
    df = pd.read_csv(path, sep="\t", header=None, names=["label", "doc_id", "sentence_id", "topic_pred_lda", "text"])

    # adjust label range (1,27) -> (0,26)
    df["label"] = df["label"] - 1
...
def get_dataset(train, valid, test, tokenizer, max_len, num_labels):
    data_train  = DatasetRisk(X=train[0], y=train[1], tokenizer=tokenizer, max_length=max_len, num_labels=num_labels)
    data_valid  = DatasetRisk(X=valid[0], y=valid[1], tokenizer=tokenizer, max_length=max_len, num_labels=num_labels)
    data_test  = DatasetRisk(X=test[0], y=test[1], tokenizer=tokenizer, max_length=max_len, num_labels=num_labels)

    return data_train, data_valid, data_test

def get_model(model_name, num_labels):
    if model_name in ["bert-base-uncased", "ProsusAI/finbert", "yiyanghkust/finbert-pretrain", "pborchert/BusinessBERT", "yannikmaassen/BusinessBERT2-v1-70000", "yannikmaassen/BusinessBERT2-v1-1000000", "yannikmaassen/BusinessBERT2-v2-78000" "yannikmaassen/BusinessBERT2-v2-1000000"]:
